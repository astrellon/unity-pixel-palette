	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
                ...
                float index = baseIndex + _PaletteOffset.y * _PaletteDims.x + _PaletteOffset.z * _PaletteDims.y + _PaletteOffset.w * _PaletteDims.z;

                float u = index % _Palette_TexelSize.z;
                float v = index / _Palette_TexelSize.z;

                fixed4 col = tex2D(_Palette, float2(u * _Palette_TexelSize.x, v * _Palette_TexelSize.y));
